...
public class FlightBookingRepository {
     ...
     *
     * @param id The  field of the Booking to be returned
    ...
    FlightBooking findByDate(Date bookingDate) {
        TypedQuery<FlightBooking> query = em.createNamedQuery(FlightBooking.FIND_BY_DATE, FlightBooking.class).setParameter("bookingDate", bookingDate); 
        return query.getSingleResult();
    }
    
    List<FlightBooking> findByFlight(Long flight_id) {
        TypedQuery<FlightBooking> query = em.createNamedQuery(FlightBooking.FIND_BY_FLIGHT, FlightBooking.class).setParameter("flight_id", flight_id); 
        return query.getResultList();
    }
    
    List<FlightBooking> findByContact(Long contact_id) {
        TypedQuery<FlightBooking> query = em.createNamedQuery(FlightBooking.FIND_BY_CONTACT, FlightBooking.class).setParameter("contact_id", contact_id); 
        return query.getResultList();
    }
    ...
    /**Booking findByDeparture(String departurePoint) {
        ...
        criteria.select(booking).where(cb.equal(booking.get("departure_point"), departurePoint));
    ...
    /**Booking findByDestination(String bookingDestination) {
        ...
        criteria.select(booking).where(cb.equal(booking.get("bookingDestination"), bookingDestination));
        return em.createQuery(criteria).getSingleResult();
    }*/
    ...
    FlightBooking create(FlightBooking booking) throws ConstraintViolationException, ValidationException, Exception {
        log.info("BookingRepository.create() - Creating " + booking.getContact() + " " + booking.getFlight() + " " + booking.getBookingDate());
    ...
    FlightBooking update(FlightBooking booking) throws ConstraintViolationException, ValidationException, Exception {
        log.info("BookingRepository.update() - Updating " + booking.getContact() + " " + booking.getFlight() + " " + booking.getBookingDate());
    ...
    FlightBooking delete(FlightBooking booking) throws Exception {
        log.info("BookingRepository.delete() - Deleting " + booking.getContact() + " " + booking.getFlight() + " " + booking.getBookingDate());
