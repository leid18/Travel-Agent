...
public class HotelBookingValidator {
     ...
     */
    void validateBooking(HotelBooking booking) throws ConstraintViolationException, ValidationException {
        ...
        if (hotelDateAlreadyExists(booking.getCustomerID(), booking.getId())) {
     ...
     */
    boolean hotelDateAlreadyExists(Long customerID, Long id) {
    	HotelBooking booking1 = null;
    	HotelBooking bookingWithID = null;
        try {
        	booking1 = crud.findByCustomerID(customerID);
        } catch (NoResultException e) {
            // ignore
        }

        if (booking1 != null && id != null ) {
            try {
            	bookingWithID = crud.findById(id);
                if (bookingWithID != null && bookingWithID.getCustomerID().equals(customerID)) {
                	booking1 = null;
                }
            } catch (NoResultException e) {
                // ignore
