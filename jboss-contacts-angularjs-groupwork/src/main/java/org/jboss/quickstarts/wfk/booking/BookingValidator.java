...
public class BookingValidator {
    ...
    @Inject
    private HotelRepository crud4;
    
    @Inject
    private FlightRepository crud5;
    /**
    ...
    void validateBooking(Booking booking) throws ConstraintViolationException, ValidationException {
        ...
        if (taxiDateAlreadyExists(booking.getTaxi().getId(), booking.getDate(), booking.getId())) {
            throw new ValidationException("Unique taxi&date Violation");
        }
        
        // Check the uniqueness of the combination of taxi and date
        if (hotelDateAlreadyExists(booking.getHotel().getId(), booking.getDate(), booking.getId())) {
            throw new ValidationException("Unique hotel&date Violation");
        }
        
        // Check the uniqueness of the combination of taxi and date
        if (flightDateAlreadyExists(booking.getFlight().getId(), booking.getDate(), booking.getId())) {
            throw new ValidationException("Unique flight&date Violation");
        }
        
        // Check whether the taxi is exist
        if (hotelNotExist(booking.getHotel().getId())) {
            throw new ValidationException("HotelNotExist");
        }
        
        // Check whether the taxi is exist
        if (taxiNotExist(booking.getTaxi().getId())) {
            throw new ValidationException("TaxiNotExist");
        }
        
        // Check whether the taxi is exist
        if (flightNotExist(booking.getFlight().getId())) {
            throw new ValidationException("FlightNotExist");
        }
     ...
     * @return boolean which represents whether the combination of taxi and date was found, and if so if it belongs to the user with id
     */    
    boolean hotelDateAlreadyExists(Long hotelID, Date date, Long id) {
        List<Booking> booking1 = null;
        Booking bookingWithID = null;
        try {
        	booking1 = crud.findByHotelID(hotelID);
        } catch (NoResultException e) {
            // ignore
        }
     ...
     * @return boolean which represents whether the combination of taxi and date was found, and if so if it belongs to the user with id
     */    
    boolean flightDateAlreadyExists(Long flightID, Date date, Long id) {
        List<Booking> booking1 = null;
        Booking bookingWithID = null;
        try {
        	booking1 = crud.findByFlightID(flightID);
        } catch (NoResultException e) {
            // ignore
        }
     ...
     * @return boolean whether the taxi is found
     */
    boolean hotelNotExist(Long hotelID){
    	Hotel hotel = null;
    	try {
    		hotel = crud4.findById(hotelID);
    	}catch (NoResultException e) {
            // ignore
        }
    	return !(hotel != null);
     ...
     * @return boolean whether the taxi is found
     */
    boolean flightNotExist(Long flightID){
    	Flight flight = null;
    	try {
    		flight = crud5.findById(flightID);
    	}catch (NoResultException e) {
            // ignore
        }
    	return !(flight != null);
